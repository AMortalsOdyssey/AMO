#!/usr/bin/env python3
"""
AMO 批量导入脚本: 1001-1261 章（人界篇最后部分）
基于 batch_import_501_1000.py，修改锚点为 ch1000 = 367 岁

用法:
  python3 scripts/import/batch_import_1001_1261.py --start 1001 --end 1050 --dry-run
  python3 scripts/import/batch_import_1001_1261.py --start 1001 --end 1261
"""
...
# 累积年份计算锚点 - ch1000 = 367 岁
BASE_CHAPTER = 1000
BASE_YEAR = 367

LOG_FILE = Path(__file__).parent / "batch_import_1001_1261.log"
    ...
    def estimate_time_passed(self, content: str) -> tuple[float, int, int]:
        ...
        for attempt in range(5):  # 增加到 5 次重试
            try:
                ...
                if not choices:
                    log.warning(f"Empty choices in LLM response (attempt {attempt+1})")
                    if attempt < 4:
                ...
                if not answer:
                    log.warning(f"Empty content in LLM response (attempt {attempt+1})")
                    if attempt < 4:
                        time.sleep(5 * (2 ** attempt))
                    continue
                ...
                if attempt < 4:
                    time.sleep(5 * (2 ** attempt))  # 5, 10, 20, 40, 80 秒
...
def init_year_tracker(base_chapter: int, base_year: int):
    """初始化年份追踪器"""
...
def run_batch_import(start_chapter: int, end_chapter: int, dry_run: bool = False,
                     ...
                     base_chapter: int = BASE_CHAPTER, base_year: int = BASE_YEAR):
    """执行批量导入"""
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = OUTPUT_DIR / f"batch_1001_1261_{run_id}"
    ...
    year_tracker = init_year_tracker(base_chapter, base_year)
    ...
    for ch_num in range(start_chapter, end_chapter + 1):
        ...
        if not content:
            ...
            stats["chapters_skipped"].append(ch_num)
            continue
        ...
        # Commit per chapter（关键：每章独立提交，避免长事务断开）
        try:
            pg_writer.commit()
        except Exception as e:
    ...
    if stats["chapters_skipped"]:
        log.info(f"  Skipped chapters: {stats['chapters_skipped']}")
...
def main():
    parser = argparse.ArgumentParser(description="AMO 批量导入: 1001-1261 章（累积年份计算）")
    parser.add_argument("--start", type=int, default=1001, help="起始章节")
    parser.add_argument("--end", type=int, default=1261, help="结束章节")
